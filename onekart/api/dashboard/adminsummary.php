...
$totalUsersQuery = $conn->query("SELECT COUNT(*) AS count FROM user");
...
$pendingOrdersQuery = $conn->query("SELECT COUNT(*) AS pending FROM orders WHERE status = 'placed'");
$pending_orders = $pendingOrdersQuery->fetch_assoc()['pending'] ?? 0;
$pendingSellersQuery = $conn->query("SELECT COUNT(*) AS pending FROM user WHERE role = 'farmer' AND status = 'pending'");
$pending_sellers = $pendingSellersQuery->fetch_assoc()['pending'] ?? 0;
