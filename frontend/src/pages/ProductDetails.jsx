    ...
    return (
        <div style={{ background: 'var(--bg-pure)', minHeight: '100vh', paddingBottom: '10rem', position: 'relative' }}>
            ...
            {relatedProducts.length > 0 && (
                <div style={{ marginTop: '8rem', padding: '10rem 0', background: 'rgba(255, 255, 255, 0.4)', backdropFilter: 'blur(30px)', borderTop: '1px solid rgba(22, 66, 60, 0.05)', position: 'relative', width: '100vw', marginLeft: '50%', transform: 'translateX(-50%)' }}>
                    ...
                    <div className="container" style={{ position: 'relative', zIndex: 1, padding: '0 2rem' }}>
                        <div style={{ textAlign: 'center', marginBottom: '4rem' }}>
                            <span style={{ fontSize: '0.75rem', fontWeight: '800', letterSpacing: '0.4em', color: 'var(--primary-light)', textTransform: 'uppercase', display: 'block', marginBottom: '1.5rem' }}>Registry Exploration</span>
                            <h2 style={{ fontSize: '3.5rem', fontFamily: '"Playfair Display", serif', color: 'var(--primary)', letterSpacing: '-0.02em' }}>Related Arrivals</h2>
                        </div>

                        <div className="product-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '2.5rem' }}>
                            {relatedProducts.slice(0, 6).map(p => (
                                <Link to={`/product/${p._id}`} key={p._id} className="glass-card hero-fade-in" style={{
                                    textDecoration: 'none',
                                    display: 'flex',
                                    ...
                                    <div className="info" style={{ padding: '0.8rem 1.2rem 1.2rem', display: 'flex', flexDirection: 'column', flex: 1 }}>
                                        <span style={{ fontSize: '0.6rem', color: 'var(--primary-light)', fontWeight: '800', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: '0.2rem' }}>{p.category}</span>
                                        <h3 style={{ fontSize: '1.2rem', marginBottom: '0.6rem', color: 'var(--primary)', fontWeight: '800', fontFamily: '"Playfair Display", serif', lineHeight: '1.2' }}>{p.name}</h3>
                                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 'auto', paddingTop: '0.8rem', borderTop: '1px solid rgba(22, 66, 60, 0.05)' }}>
                                            <div style={{ fontWeight: '900', fontSize: '1.2rem', color: 'var(--primary)', letterSpacing: '-0.02em' }}>₹{p.price}</div>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '4px', opacity: 0.8 }}>
                                                <Star size={12} fill="#fbbf24" color="#fbbf24" />
                                                <span style={{ fontSize: '0.75rem', fontWeight: '800', color: 'var(--primary)' }}>{p.rating || '4.9'}</span>
                                            </div>
                                        </div>
                                    </div>
                                </Link>
